...
- Install the library: `npm install --save docker-code-sandbox`
- Pull the docker image used by the library: `docker pull apirobot/docker-sandbox`
...
mySandbox.initialize(err => {
  ...
  mySandbox.run({code, language, timeoutMs}, (err, result) => {
