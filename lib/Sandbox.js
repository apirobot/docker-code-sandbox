...
const defaultOptions = {
  "poolSize": 1,
  "memoryLimitMb": 50,
  "imageName": "apirobot/docker-sandbox",
  "timeoutMs": 10000
};
...
class Sandbox {
  ...
  run (options, cb) {
    if (!options.code || !options.language) {
      throw new Error("'code' and 'language' parameters are mandatory");
    }

    if (SUPPORTED_LANGUAGES.indexOf(options.language) < 0) {
      throw new Error("The language " + options.language + " is not supported");
    }

    let code = options.code
    let stdin = options.stdin || undefined
    let language = options.language
    let timeoutMs = options.timeoutMs || this.options.timeoutMs

    if (!code || !_.isString(code))
      throw new Error("Please provide the code to run as a string or an object {code: xxx}")

    const job = new Job(code, language, stdin, timeoutMs, cb)
